...
def tf_str_len(s):
    ...
    """
    return tf.size(input=tf.compat.v1.string_split([s],""))

def tf_rank1_tensor_len(t):
    """
    Returns the length of a rank 1 tensor t as rank 0 int32
    """
    l = tf.reduce_sum(input_tensor=tf.sign(tf.abs(t)), axis=0)
...
def tf_seq_to_tensor(s):
    ...
    return tf.strings.to_number(
        tf.sparse.to_dense(tf.compat.v1.string_split([s],","), default_value='0'), out_type=tf.int32
    )[0]
...
def smart_length(length, bucket_bounds=tf.constant([128, 256])):
    ...
    key = tf.cast(tf.reduce_sum(input_tensor=greater), tf.int64)
